function usageSourceCode(v){const x=T(v).toLowerCase().replace(/[\s_-]+/g,'');if(x==='stock'||x==='stockmovement'||x==='movement')return'stock';if(x==='itemsale'||x==='sale'||x==='sales')return'item_sale';return x==='bom'||x==='bomb'?'bom':''}
function usageSettingsMap(){const m=new Map();for(const x of Array.isArray(S.usageSource)?S.usageSource:[]){const rm=T(x.rm_code).toUpperCase(),source=usageSourceCode(x.source);if(rm&&source)m.set(rm,{source,cup_size:T(x.cup_size).toUpperCase()})}return m}
function itemSaleUsage(branch,date){const manual=new Map();for(const x of Array.isArray(S.cupMapping)?S.cupMapping:[]){const code=T(x.item_code).toUpperCase();if(code)manual.set(code,x)}const bySize=new Map(),byRm=new Map();for(const r of S.item||[]){if(T(V(r,'wh'))!==branch||(date&&T(V(r,'date'))!==date))continue;const code=T(V(r,'item')).toUpperCase(),name=T(V(r,'itemd')),x=manual.get(code),match=name.match(/\((P|PB|PP|M|MB|L|LB|H|S)\)\s*$/i),autoSize=match?match[1].toUpperCase():'',count=x?x.count_as_cup!==false:/^KR\d+/i.test(code)&&!!autoSize,size=T(x?.cup_size||autoSize).toUpperCase(),rm=T(x?.usage_rm_code).toUpperCase(),qty=N(V(r,'qty'));if(!count||!qty)continue;if(size)bySize.set(size,(bySize.get(size)||0)+qty);if(rm)byRm.set(rm,(byRm.get(rm)||0)+qty)}return{bySize,byRm}}
function compute(){
 ...
 requestAnimationFrame(()=>{
  const exp=expected(b,d),mov=IDX.stockByBranch.get(b)||[],out=[],needs=[],groups=new Map(),usageSettings=usageSettingsMap(),itemUsage=itemSaleUsage(b,d);
  ...
  for(const [rm,rs] of groups){
   ...
   const usageCfg=usageSettings.get(T(rm).toUpperCase())||null,usageSource=!usageCfg?'ยังไม่กำหนด':usageCfg.source==='stock'?'Stock Movement':usageCfg.source==='item_sale'?'Item Sale':'BOM',usageCupSize=usageCfg?.cup_size||'',mappedItemUsage=itemUsage.byRm.get(T(rm).toUpperCase());
   const usageSetupNeeded=!usageCfg||(usageCfg.source==='item_sale'&&mappedItemUsage===undefined&&!usageCupSize),usageNeedConversion=usageCfg?.source==='bom'&&need;
   const usageValue=!usageCfg?null:usageCfg.source==='stock'?movSales:usageCfg.source==='item_sale'?(mappedItemUsage!==undefined?mappedItemUsage:(usageSetupNeeded?null:(itemUsage.bySize.get(usageCupSize)||0))):bom;
   const calc=usageSetupNeeded||usageNeedConversion?null:bf+gr+tin+wipin+adjin-rtv-tout-wipout-damage-adjout-(usageValue||0)-waste;
   const cv=localStorage.getItem(countsKey(b,rm)),count=cv===null?null:Number(cv),diff=count===null||calc===null?null:count-calc,status=usageSetupNeeded?'Needs usage setup':usageNeedConversion?'Needs conversion':count===null?'Not counted':Math.abs(diff)<0.0005?'OK':diff<0?'Short':'Over';
   const cost=rs.map(x=>N(V(x,'cost'))).find(v=>v!==0)||N(V(r,'cost'));
   out.push({rm,desc,unit:su,bf,gr,tin,wipin,adjin,rtv,tout,wipout,damage,adjout,bom,bomRaw,hasBom,missingConv:missing.length,mov:movSales,uv:bom===null?null:movSales-bom,usageSource,usageCupSize,usageValue,usageSetupNeeded,waste,calc,reportTheory,reportActual,count,diff,diffp:diff===null||calc===null||Math.abs(calc)<1e-9?null:diff/calc*100,cost,vc:diff===null?0:diff*cost,status,sourceRows:rs.length});
  }
