const cols=['RM','Description','Unit','T.Out','WIP Out','Damage','Adj.Out','Waste','Usage Source','Usage','Calc Theory','Report Theory','Report Actual','Actual Count','Count Diff','Diff %','Std.Cost','Variance Cost','Status'];
...
function render(){
 ...
 const exp=S.currentExpected||new Map(),usageConfigured=S.analysis.filter(x=>x.usageSource!=='ยังไม่กำหนด').length;
 $('#kItems').textContent=S.analysis.length;$('#kUsage').textContent=usageConfigured;$('#kShort').textContent=S.analysis.filter(x=>x.status==='Short').length;$('#kOver').textContent=S.analysis.filter(x=>x.status==='Over').length;const kc=$('#kConv');if(kc)kc.textContent=new Set(S.needs.map(x=>x.rm)).size;$('#kCost').textContent=F(S.analysis.reduce((z,x)=>z+x.vc,0));
