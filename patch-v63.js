...
(() => {
  ...
  function exportJpg(){
    if(!auditState.branch){toastMsg('กรุณากรอกชื่อสาขาก่อน Export','warn');return}
    const rows=exportRows(), meta=exportMeta(); const PAGE_W=2480,PAGE_H=1754,M=70,HEAD=370,ROW_H=62,HEADER_H=58; const cols=[70,390,500,230,360,360,260]; const perPage=Math.floor((PAGE_H-M-HEAD-HEADER_H-80)/ROW_H); const pages=Math.max(1,Math.ceil(rows.length/perPage));
